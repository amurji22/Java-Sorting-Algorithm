...
public class SortingHubController implements Initializable {
    ...
    public void setSortStrategy(){
        if(Combo_Box.getValue().equals("Merge Sort")){
            sortingMethod = new MergeSort(this);
            new Thread(()-> sortingMethod.sort(intarray, 0, getSize()-1)).start();
        }
        if(Combo_Box.getValue().equals("Quick Sort")){
